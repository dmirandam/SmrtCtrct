App = {
  ...
  loadContract: async () => {
    try {
      ...
      App.contracts.Tareas = TruffleContract(TareasJSON);
      App.contracts.Tareas.setProvider(App.web3Provider);

      App.Tareas = await App.contracts.Tareas.deployed();
    } catch (error) {
      console.error(error);
    }
  },
  ...
  renderTasks: async () => {
    const tasksCounter = await App.Tareas.tasksCounter();
    const taskCounterNumber = tasksCounter.toNumber();

    let html = "";

    for (let i = 1; i <= taskCounterNumber; i++) {
      const task = await App.Tareas.tasks(i);
  ...
  createTask: async (title, description) => {
    try {
      const result = await App.Tareas.createTask(title, description, {
        from: App.account,
      });
  ...
  toggleDone: async (element) => {
    ...
    await App.Tareas.toggleDone(taskId, {
      from: App.account,
    });
    window.location.reload();
  },
